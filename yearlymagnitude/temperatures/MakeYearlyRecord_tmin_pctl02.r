...
metdata <- brick("/proj/bolinc/users/x_aidjo/Disasters/tmin_modernpctl.nc",varname="pctl") #load meteorological data
...
while (end < "2018-01-01") {
  ...
  for (state in 1:256) {
    ...
    global_temp[state,years[t]] <- sum(extr<2.275)/nrow(extr)
...
## Save the data frame
save(global_temp,file="CDHR2019_tmin_pctl02.rda")
