...
global_temp <- st_read("GADM/gadm36_0.shp")
...
metdata <- brick("/proj/bolinc/users/x_aidjo/Disasters/tmin_anom.nc",varname="mn2t")
...
while (end < "2018-01-01") {
  ...
  for (state in 1:256) {
    ...
    global_temp[state,years[t]] <- sum(extr)/nrow(extr)
...
## Save the data frame
save(global_temp,file="CDHR2019_tminanom.rda")
