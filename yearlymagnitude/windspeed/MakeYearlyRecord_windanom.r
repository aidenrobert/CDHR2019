...
metdata <- brick("/proj/bolinc/users/x_aidjo/Disasters/windspeed_modern_anom.nc",varname="U10")
...
while (end < "2018-01-01") {
  ...
  for (state in 1:256) {
    ...
    global_wind[state,years[t]] <- sum(extr)/nrow(extr)
...
## Save the data frame
save(global_wind,file="CDHR2019_windspeed_anom.rda")
